...
case "$build" in
  cranelift|cranelift-min|pulley|pulley-min) ;;
  *)
    echo "unknown build mode: $build" >&2
    usage >&2
    exit 1
    ;;
esac
...
if [[ "$build" == *-min ]]; then
  ...
  cmake_flags=(
    ...
    "-DWASMTIME_USER_CARGO_BUILD_OPTIONS:LIST=${build_std[*]}"
  )
  flags=(--no-default-features --features disable-logging)
  ...
  if [[ "$build" == "cranelift-min" ]]; then
    ...
    flags+=(--features "$cli_feat_list")
    cmake_flags+=(
      -DWASMTIME_FEATURE_ALL_ARCH=ON
      -DWASMTIME_FEATURE_PULLEY=ON
      ...
      -DWASMTIME_FEATURE_COMPONENT_MODEL_ASYNC=ON
    )
  ...
  if [[ "$build" == "pulley" ]]; then
    ...
    cmake_flags+=(-DWASMTIME_FEATURE_COMPONENT_MODEL=ON)
  fi
fi
