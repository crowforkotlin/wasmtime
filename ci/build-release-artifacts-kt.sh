...
if [[ "$build" == *-min ]]; then
  ...
  if [[ "$build" == "cranelift-min" ]]; then
    ...
    # 1. CLI Flags
    cli_feat_list="run,pulley,gc,gc-drc,pooling-allocator"
    flags="$cli_base_features --features $cli_feat_list"
  ...
  if [[ "$build" == "pulley" ]]; then
    flags="--features all-arch,pulley"
...
# 【新增】针对 GNU (MinGW) 的处理
  ...
  # 强制 CMake 和 Rust 使用 GCC/G++，防止意外调用 MSVC 的 cl.exe
  export CC=gcc
  export CXX=g++

  # 【可选】如果你希望生成静态链接的库（类似 .lib），不依赖 libgcc_s_seh-1.dll 等
